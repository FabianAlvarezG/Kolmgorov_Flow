...
def launch(
    ...
    if type(base_dir) == type(None):
        if hostname.startswith('alan') or hostname.startswith('uran'):
            base_dir = '/scratch01.local/falvarez/Kflow'
    ...
    print(base_dir)
    work_dir = os.path.join(base_dir, 'N{0:0>4d}_rxy{1}_ryz{2}_test'.format(N, ratio_xy, ratio_yz))
    args += ['--wd', work_dir]
    ...
    c.launch(args = args + [
        ...
        #'--src-wd','/home/falvarez/Documents/rdt/initial_condition',
        #'--src-iteration','0'
        ])
    return None

if __name__ == '__main__':
    launch(ratio_yz = 8,niterations = '1024')
