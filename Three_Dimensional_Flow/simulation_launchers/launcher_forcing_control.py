...
def launch(
    ...
    if type(base_dir) == type(None):
        if hostname.startswith('alan') or hostname.startswith('uran'):
            base_dir = '/scratch03.local/falvarez/Kflow/forcing_control/detail_high_reynolds/ryz{0}'.format(ratio_yz)
...
if __name__ == '__main__':
    ...
    #launch(N = 48, ratio_yz = 2, niterations = 128)
        launch(N = 128, ratio_yz = 4, niterations = 163840, famplitude=f)
