...
def launch(
    ...
    if type(base_dir) == type(None):
        if hostname.startswith('alan') or hostname.startswith('uran'):
            base_dir = '/scratch03.local/falvarez/Kflow/forcing_control/extra_preciosion_detailed_high_reynolds/ryz{0}'.format(ratio_yz)
...
if __name__ == '__main__':
    Re = np.linspace(250,265,32)
    forcing = Re**2/(2*np.pi)**3*0.25
    ffor i,f in enumerate(forcing):
    #launch(N = 48, ratio_yz = 1, niterations = 128)
    #launch(N = 48, ratio_yz = 2, niterations = 128)
        launch(N = 256, ratio_yz = 4, niterations = 1638400, famplitude=f, index=i)
