import os
import h5py
from simulator_averagex import simulator
from TurTLE import host_info
import getpass

from simulator_averagex import username, homefolder, hostname
...
def launch(
    ...
    if type(base_dir) == type(None):
        if hostname.startswith('alan') or hostname.startswith('uran'):
            base_dir = '/scratch03.local/falvarez/Kflow'
    ...
    print(base_dir)
    work_dir = os.path.join(base_dir, 'N{0:0>4d}_averagex'.format(N))
    args += ['--wd', work_dir]
