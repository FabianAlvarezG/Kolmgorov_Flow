...
def launch(
    ...
    if type(base_dir) == type(None):
        if hostname.startswith('alan') or hostname.startswith('uran'):
            base_dir = '/scratch02.local/falvarez/Kflow/forcing_control/very_high_reynolds/ryz{0}'.format(ratio_yz)
    ...
    dt = 0.01/N
    assert(Nz % 2 == 0)
    src_dir = '/scratch01.local/falvarez/Kflow/forcing_control/very_high_reynolds/ryz{0}'.format(ratio_yz)
    src_dir_sim = os.path.join(src_dir, 'N{0:0>4d}_rxy{1}_ryz{2}_famplitude_{3:.3f}'.format(N, ratio_xy, ratio_yz,famplitude))
    c.launch(args = args + [
        ...
        '--niter_DB', '{0}'.format(niterations),
        '--dt', '{0}'.format(dt),
        '--simname', simname,
        '--forcing_type', 'Kolmogorov_and_drag',
        '--njobs', '10',
        '--fftw_plan_rigor', 'FFTW_MEASURE',
        '--src-wd','{0}'.format(src_dir_sim),
        '--src-simname','{0}'.format(simname),
        '--src-iteration','819200',
        '--overwrite-src-parameters'])
    return None
import numpy as np
if __name__ == '__main__':
    Re = np.logspace(8,10,num=32,base=2.0)
    forcing = Re**2/(2*np.pi)**3*0.25
    for f in forcing:
    #launch(N = 48, ratio_yz = 1, niterations = 128)
    #launch(N = 48, ratio_yz = 2, niterations = 128)
        launch(N = 128, ratio_yz = 4, niterations = 81920, famplitude=f)
