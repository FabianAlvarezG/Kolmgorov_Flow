...
def launch(
    ...
    if type(base_dir) == type(None):
        if hostname.startswith('alan') or hostname.startswith('uran'):
            base_dir = '/scratch02.local/falvarez/Kflow/forcing_control/ryz{0}'.format(ratio_yz)
    ...
    dt = 0.009/N
    assert(Nz % 2 == 0)
    src_dir = '/scratch01.local/falvarez/Kflow/forcing_control/ryz{0}'.format(ratio_yz)
