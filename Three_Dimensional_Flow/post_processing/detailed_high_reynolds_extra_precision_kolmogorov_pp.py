...
def main():
    Reynolds =np.linspace(250,265,16)
    ...
    for i,f in enumerate(forcing_amplitudes):
        ...
        work_dir = os.path.join(path,'N{0:0>4d}_rxy{1}_ryz{2}_famplitude_{3}'.format(N,rxy,ryz,i))
    ...
    fig.savefig('RevsA_raw_detailed_high_reynolds_extra_precision_scratch0{0}_ryz{1}.png'.format(scratch, ryz), format='png')
    plt.clf()
    fig, ax = plt.subplots(nrows=1,ncols=1,figsize=(10,7))
    ...
    ax.set_title(r'$N_z=$'+'{0}'.format(int(N/ryz)))
    fig.savefig('RevsA_raw_energy_detailed_high_reynolds_extra_precision_scratch0{0}_ryz{1}.png'.format(scratch, ryz), format='png')
    return 0
