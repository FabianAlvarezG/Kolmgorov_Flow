...
for i,f in enumerate(forcing_amplitudes):
    ...
    if path_h5_file in file:
        ...
        varphi = np.fft.irfftn(varphi_k, axes=(0,1,2))*nx*ny

        amplitudes_0[i] = abs(InnerProduct(varphi, psi_0))*np.sqrt(2)*(0.5/f)**2
        amplitudes_a0[i] = abs(InnerProduct(varphi, psi_a0))*(0.5/f)**2
        print(amplitudes_0[i],amplitudes_a0[i])
    else:
...
fig.savefig('RevsA_raw_stress_projection_high_reynolds_scratch0{0}_ryz{1}.png'.format(scratch, ryz), format='png')
