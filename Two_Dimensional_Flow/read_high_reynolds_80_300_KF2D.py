...
#famplitude = forcing_amplitudes[index]
    ## Saving directories
    R = Reynolds[i]
    simname = 'N{0:0>4d}_rxy{1}_famplitude_{2}'.format(N, ratio_xy,i+1)
    base_dir = '/scratch0{0}.local/falvarez/Kflow/2D/high_reynolds'.format(scratch)
    work_dir = os.path.join(base_dir, 'N{0:0>4d}_rxy{1}_famplitude_{2}'.format(N, ratio_xy,i+1))
    ...
    print(famplitude,i, amplitudes_0[i], amplitudes_a0[i])
plt.clf()
data = np.zeros((32, 3))
data[:,0] = Reynolds
data[:,1] = amplitudes_0
data[:,2] = amplitudes_a0
np.savetxt("high_reynolds_amplitudes_scratch{0}.csv".format(scratch), data, delimiter=",")

### Load Previous data

data_old = np.loadtxt('high_reynolds_amplitudes.csv',delimiter=",")
Reynolds_old = data_old[:,0]
amplitudes_0_old = data_old[:,1]
amplitudes_a0_old = data_old[:,2]

Reynolds_new = np.union1d(Reynolds_old[Reynolds_old<140],Reynolds)
amplitudes_0_new = np.union1d(amplitudes_0_old[Reynolds_old<140],amplitudes_0)
amplitudes_a0_new = np.union1d(amplitudes_a0_old[Reynolds_old<140],amplitudes_a0)

fig, ax = plt.subplots(nrows=1,ncols=1,figsize=(10,7))
#plt.plot(Reynolds, re9lative_amplitude_a0,'o', color='black')
...
#plt.ylim((0,1.2))
ax.plot(Reynolds_new, amplitudes_0_new,'--', color='red',marker='s',label=r'$|\langle \psi_0 | \psi \rangle |$')
ax.plot(Reynolds_new, amplitudes_a0_new,'--', color='blue', marker='s',label=r'$|\langle \psi_\alpha | \psi \rangle |$')
...
#plt.ylim(0,1.2)
plt.savefig('RevsA_raw_high_reynolds_scratch0{0}_2D.png'.format(scratch), format='png')
