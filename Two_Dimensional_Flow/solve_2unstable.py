...
#mat = matrix(2, 0,1)from scipy.optimize import fsolve
import numpy as np
import matplotlib.pyplot as plt
plt.rcParams.update({
    'text.usetex' : False,
...
#ax.plot(alphas, pred_2(alphas, 0.0),'-', color='red', linewidth=4)
ax.plot(alphas, Rc,'o', color='green', label='Numerical result using {0} modes'.format(N))
ax.plot(alphas2, Rc2,'o', color='blue', label='Numerical result using {0} modes'.format(N))
ax.plot(alphas, pred_2(alphas, mu),'--', color='orange', linewidth=4, label=r'$Rc(\alpha, \mu)$')
ax.plot(alphas2, pred_2(2*alphas2, mu),'--', color='red', linewidth=4, label=r'$Rc(2\alpha, \mu)$')
...
ax.set_ylim((0,8))
ax.set_xlim((0,1))
fig.savefig('Critical_Reynolds_Nmodes_{0}_mu_{1}_2unstable.png'.format(N, mu), format='png')
#mat = matrix(2, 0,1)
